...
> Run this from a PowerShell prompt, not Git Bash/MSYS/Cygwin — `install.sh` will detect those shells and point you here instead. Requires `git` and Python 3.9+ on PATH. The script also installs [`windows-curses`](https://pypi.org/project/windows-curses/) into the venv, since Python's stdlib `curses` module (used by the TUI) doesn't exist on Windows.
